...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // fg[0] is the sum of cost terms 
    fg[0] = 0; 

    // constraint to reference state 
    for(int i=0; i<N; i++)
    {
    ...
    // reduce the difference of delta and a
    for(int i=0; i< N-2; i++)
    {
    ...
    // Set up constraints 
    // fg[1~n_total] constraints generated by variables 
	...
	AD<double> dy = coeffs[1] + 2 * coeffs[2] * x0 + 3 * coeffs[3] * x0 * x0; 
	// AD<double> dy = coeffs[1] + 2 * coeffs[2] * x0; 

	AD<double> psides0 = CppAD::atan(dy); 
	...
	fg[1+epsi_start+i] = epsi1 - (psi0 - psides0 + v0 * dt/Lf * dt); 
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  return {solution.x[delta_start], solution.x[a_start], solution.x[v_start+1]};
}
