...
vector<CVehicle> predict(vector<vector<double> >& sensor_fusion, double t)
{
	...
	vehicle.move_at_constant_speed(t); 
  ...
  h.onMessage([&ref_vel, &lane, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
		...
		// check all the other cars' future position 
...
/*
		for(int i=0; i < sensor_fusion.size(); i++)
		{
			...
			check_car_s += (double)prev_size*0.02*check_car_v;
...
*/
		...
		{
		    /*
		       double next_s = car_s + (i+1) * dis_inc; 
		       double next_d = car_d; 
		       ...
		       */
		    // next_x_vals.push_back(car_x + i* dis_inc * cos(deg2rad(car_yaw)));
		    // next_y_vals.push_back(car_y + i* dis_inc * sin(deg2rad(car_yaw)));
		}    
		// transform into body reference 
		double ref_x = car_x; 
		double ref_y = car_y; 
		...
		// if prev size is almost empty, use the car as start reference 
		if(prev_size < 2)
		{
